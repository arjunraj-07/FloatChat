...
function buildScene(THREE, target) {
  ...
  const column = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ map: gradientTexture(THREE, [[0, '#0f4956'], [0.45, '#093039'], [1, '#04121a']]), side: THREE.BackSide, transparent: true, opacity: 0.96, depthWrite: false })
  );
  ...
  band.position.y = 1.1; float.add(band);
  const band2 = band.clone(); band2.position.y = -1.0; float.add(band2);
  ...
  /* Interior particulate — gives the cabin air depth and reacts to the parallax. */
  const MOTES = 150;
  ...
  return { scene, mapPlane, ocean, underside, column, float, cabin, panel, bubbles, bubbleData: { pos, vel, seed, COUNT }, gaugeAnchors, stripAnchor, glow, motes, moteData: { mp, mv, MOTES }, lampMat };
}

function DiveStage({ p, onSettled }) {
  const host = React.useRef(null);
  const anchors = React.useRef({ gauges: [], strip: null });
  const [ready, setReady] = React.useState(false);

  React.useEffect(() => {
    const THREE = window.THREE;
    const el = host.current;
    if (!THREE || !el) { onSettled({ gauges: [], strip: null }); return; }

    const tx = p.longitude, tz = -p.latitude;
    ...
    const frame = (now) => {
      ...
      if (t >= crossT) {
        if (!built.bubbles.visible) { built.bubbles.visible = true; built.bubbles.material.opacity = 0.9; }
  ...
  return <div className="fc-dive-gl" ref={host} data-ready={ready}></div>;
}

function DiveView({ p, onClose }) {
  const [settled, setSettled] = React.useState(false);
  const [anchors, setAnchors] = React.useState({ gauges: [], strip: null });
  ...
  React.useEffect(() => { if (settled) closeRef.current?.focus(); }, [settled]);

  const onSettled = React.useCallback((a) => { setAnchors(a); setSettled(true); }, []);
  const g = anchors.gauges || [];

  return (
    <div className={'fc-dive' + (settled ? ' is-settled' : '')} role="dialog" aria-modal="true" aria-label={'Inside float ' + p.platform}>
      <DiveStage p={p} onSettled={onSettled} />

      <div className="fc-dive-hud" aria-hidden={!settled}>
        {g[0] && <div className="fc-dive-val" style={{ left: g[0].x + '%', top: g[0].y + '%' }}>
          <span className="fc-dive-val-n">{st?.sst != null ? st.sst.toFixed(2) : '—'}</span><span className="fc-dive-val-u">°C</span>
        </div>}
        {g[1] && <div className="fc-dive-val" style={{ left: g[1].x + '%', top: g[1].y + '%' }}>
          <span className="fc-dive-val-n">{st?.salSurface != null ? st.salSurface.toFixed(2) : '—'}</span><span className="fc-dive-val-u">PSU</span>
        </div>}
        {anchors.strip && <div className="fc-dive-strip" style={{ left: anchors.strip.x + '%', top: anchors.strip.y + '%' }}>
          <Mono className="fc-dive-strip-id">{p.platform} · cycle {p.cycle}</Mono>
        </div>}
      </div>

      <div className="fc-dive-readout" aria-hidden={!settled}>
        <header className="fc-dive-head">
          <div><Kicker>Inside float</Kicker><Mono className="fc-dive-id">{p.profile_id}</Mono></div>
          <button ref={closeRef} className="fc-dive-close" onClick={onClose}>Surface ↑</button>
        </header>
        ...
        <Mono className="fc-xs fc-muted fc-dive-note">Instrument readout · press Esc to surface</Mono>
      </div>
    </div>
  );
}

Object.assign(window, { DiveView, DiveStage });
